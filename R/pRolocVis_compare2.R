...
pRolocVis_compare2 <- function(object, fcol,
                               foi,
                               fig.height = "600px",
                               fig.width = "100%",
                               legend.width = "200%",
    ...
    if (!hasCommonFeatureNames(object)) {
    ...
    if (missing(foi) & missing(fcol)) 
      fcol <- "markers"
    if (!missing(fcol)) {
      if (!fcol %in% fvarLabels(object[[1]])) {
    ...
    ## Datasets must contain same number of columns to use re-map?
    if (remap) {
      colNum <- unlist(lapply(object, ncol))
      if (length(unique(colNum)) != 1) 
        stop("Different number of columns between datasets")
      tot <- length(unique(unlist(lapply(object, sampleNames))))
      if (tot > ncol(object[[1]])) {
        warning("Different sample names in MSnSet's, re-naming to common sample names to enable re-mapping")
        newObj <- vector("list", length(object))
        for (i in 1:length(newObj)) {
          newObj[[i]] <- object[[i]]
          sampleNames(newObj[[i]]) <- 1:ncol(newObj[[i]])
        }
        object <- MSnSetList(newObj)
      }
    }

    
    #       tf <- unlist(lapply(object, function(z) !fcol %in% fvarLabels(z)))
    #       if (any(tf)) {
    
    #       ind <- which(tf)
    #       stop("fcol missing in fData in MSnSetList item ", ind)
...
#     }
    ...
    ## There can't be more than 12 columns in the DT table (total across both tables)
    ## so we will keep 6 for dataset 1 and 6 columns for dataset 2
    narrowedObj <- vector("list", 2)
    for (i in 1:length(object)) {
      if (missing(fdataInd)) {
        if ((nfd <- length(fvarLabels(object[[i]]))) > 6) {
          message("There can't be more than 6 feature variables per dataset. Using 3 first and last.")
          narrowedObj[[i]] <- narrowFeatureData(object[[i]], n1 = 3, n2 = 3, fcol = fcol)
        }
      } else {
        if(length(fdataInd) > 6)
          stop("There can't be more than 6 feature variables, check fdataInd")
        narrowedObj[[i]] <- selectFeatureData(object[[i]], fcol = fdataInd)
      }
      quantData[[i]] <- exprs(narrowedObj[[i]])
    }
    object <- MSnSetList(narrowedObj)
