...
pRolocVis_compare <- function(object, fcol1, fcol2,
  ...
  ##starting setting for the app
  fd1 <- fData(object[[1]])
  fd2 <- fData(object[[2]])
  # fd <- lapply(object@x, function(x) fData(x))   # <---why does this not work?
  # toSel <- 1:nrow(object[[1]])
  feats <- toSel <- featureNames(object[[1]])
  # names(feats) <- feats
  idxDT <- numeric()
  namesIdxDT <- character()
    ...
    fluidPage(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          checkboxInput("checkbox", label = "Show labels", value = FALSE),
          br(),
  ...
  server <-
    function(input, output, session) {
      ...
      output$spatialmap1 <- renderPlot({
        ...
        # if (length(idxDT)) {
        #   highlightOnPlot(object_coords[[1]], idxDT, cex = 1.3)
        #   if (input$checkbox) 
        #     highlightOnPlot(object_coords[[1]], idxDT, labels = TRUE, pos = 3)
        # }
        ...
        if (!is.null(input$markers)) {
          for (i in 1:length(input$markers))
            points(object_coords[[1]][mrkSel1()[[i]], ], pch = 21,
        ...
        if (length(idxDT)) {
          # .highlight(object_coords[[1]], namesIdxDT)
          highlightOnPlot(object_coords[[1]], namesIdxDT, cex = 1.3)
          if (input$checkbox)
            # .highlight(object_coords[[1]], namesIdxDT, labels = TRUE)
            highlightOnPlot(object_coords[[1]], namesIdxDT, labels = TRUE, pos = 3)
        }
      ...
      output$spatialmap2 <- renderPlot({
        ...
        # if (length(idxDT)) {
        #   highlightOnPlot(object_coords[[2]], idxDT, cex = 1.3)
        #   if (input$checkbox) 
        #     highlightOnPlot(object_coords[[2]], idxDT, labels = TRUE, pos = 3)
        # }
        # resetLabels$logical <<- FALSE
        ...
        if (!is.null(input$markers)) {
          for (i in 1:length(input$markers))
            points(object_coords[[2]][mrkSel2()[[i]], ], pch = 21,
                   cex = 1.4, bg = myCols()[i], col = myCols.bg()[i])
        }
        idxDT <<- feats[input$fDataTable_rows_selected] ## highlight point on plot by selecting item in table
        if (resetLabels$logical) idxDT <<- numeric()  ## If TRUE labels are cleared
        namesIdxDT <<- names(idxDT)
        if (length(idxDT)) {
          # .highlight(object_coords[[2]], namesIdxDT)
          highlightOnPlot(object_coords[[2]], namesIdxDT, cex = 1.3)
          if (input$checkbox)
            # .highlight(object_coords[[2]], namesIdxDT, labels = TRUE)
            highlightOnPlot(object_coords[[2]], namesIdxDT, labels = TRUE, pos = 3)
        }
        resetLabels$logical <- FALSE
        height <- reactive(ifelse(!is.null(input$innerWidth),input$innerWidth*3/5,0)) # fix ratio 1:1
      ...
      output$fDataTable <- DT::renderDataTable({
        # browser()
        feats <<- unique(c((which(brushedProts1$i & brushedProts1$j)),
                           (which(brushedProts2$i & brushedProts2$j))))
        ...
        if (!is.null(input$dblClick1)) {
          ...
          idxPlot <- names(which(dist == min(dist)))
        ...
        namesIdxDT <<- names(idxDT)
        .toSel1 <- match(namesIdxDT, rownames(fd1)[brushedProts1$i & brushedProts1$j]) ## selection to highlight in DT
        .toSel2 <- match(namesIdxDT, rownames(fd2)[brushedProts2$i & brushedProts2$j]) ## selection to highlight in DT
        ...
        .dt1 <- fd1
        # .dt1 <- fd1[, -grep(mName, colnames(fd1))]
        .dt1 <- .dt1[brushedProts1$i & brushedProts1$j, input$selTab1, drop = FALSE]
        
        .dt2 <- fd2
        # .dt2 <- fd2[, -grep(mName, colnames(fd2))]
        .dt2 <- .dt2[brushedProts2$i & brushedProts2$j, input$selTab2, drop = FALSE]
