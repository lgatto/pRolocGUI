...
changes in protein localisation in different conditions. *Note: this
is currently being re-developed and will be updated in the coming months.*
...
class `MSnSet` (or a list of `MSnSet` objects of
...
argument `fcol` can be used which allows the user to specify the 
feature meta-data label (`fData` column name) to be plotted. 
The default is `markers` (i.e. the labelled data) for all 
applications except the classification app. For
...
* The comparison application (currently being re-implemented) 
may be of interest if a user wishes to 
...
click/unclick on the corresponding entry in the table or on the 
correspinding highlighted point on the interactive PCA plot. If a protein(s)
...
clicking the tab at the top of the page. The protein profiles plot 
shows the quantitation data that is stored in the `exprs` data slot 
...
examine two replicate experiments, or two experiments from different
conditions etc.  Two PCA plots are loaded side-by-side and one can
search and identify common proteins between the two data sets. *Note:
This application is currently being re-developed and a newer version
will appear shortly.*
...
the display selection table and then clicking the protein of interest on 
...
See `vignette("pRolocGUI-legacy")` for full details on how to use this 
application.
...
Lilley K.S. A foundation for reliable spatial proteomics 
data analysis, Mol Cell Proteomics. 2014 Aug;13(8):1937-52

Christoforou A., Mulvey C.M., Breckels L.M., Hayward P.C., Geladaki E., 
