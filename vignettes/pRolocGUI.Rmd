...
it will be the last for `pRolocComp`.
...
and column `unique1` which comprises `4` features are selected</p>
...
(`Data`) and `query` as well. Besides that you are also able to simply blind 
...
will clear all features (also these of `PCA`, `protein profiles` (and `Data`).
Of course, we can also add other features: If we
