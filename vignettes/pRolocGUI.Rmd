...
Thomas Naake (thomas.naake@merkur.uni-freiburg.de) in case you have any questions, suggestions or 
...
**_marker level 1_** and **_marker level 2_** which are located in the section
**_Summary matrix_** in the sidebar panel (enter `?FeatComp` to retrieve more 
...
both for **_marker level 1_** and **_marker level 2_** the matrix will consist 
...
**_Selection_**. It is made by selecting a marker via the drop-down menu 
...
only shown when the features are not already stored internally). 

<img src="figures/Screenshot_Data3.png" width="100%" alt=""></img> <p 
align="center"><Fig. 4: Appearance of `Data` tab in `pRolocComp`. The row 
`ER` and column `unique1` which comprises `4` features is selected ></p>
...
section **_Submit MSnSets_**. The subset is made on the basis of the feature 
...
</center> <p align="center">Fig. 6: Radio buttons to select `MSnSet` in 
...
these categories will be displayed in bold in the summary matrix. 

When submitting features from the columns `unique1` and `unique2`, the feature 
names will only be saved internally to the correspendent `MSnSet` and displayed 
