...
defininitions of `MSnbase` [[1](#id1)] and on the functions defined in
the package `pRoloc` [[2](#id2)]. It is especially meant for the
analyses of LOPIT [[3](#id3)] or PCP [[4](#id4)] experiments.  To
achieve reactivity and interactivity, `pRolocGUI` relies on the
...
We recomment some familiarity with the `MSnSet` class (see `?MSnSet`
...
Currently, `pRolocGUI` distributes the `pRolocVis` application. Lauch
it by load the `pRolocGUI` package and running `pRolocVis()` function:
...
`pRolocdata` experiment package) to `pRolocVis` by entering
...
[*Display selection* widget](#display) for further details).
...
other `MSnSet`.  Currently you may choose between three example
`MSnSets` (derived from experiments `andy2011` from Breckels
et al. (2013) [[5]](#id5), `dunkley2006` from Dunkley et al. (2006)
[[3]](#id3), and `tan2009r1`, the first replicate from Tan et
al. (2009) [[6]](#id6), all provided by the package `pRolocdata`) or
to use an `MSnSet` of your choice (own data).  Clicking on
**_Browse..._** will open a dialog window with which you can select a
file containing a saved `MSnSet` and load it to `pRolovVis`.
...
The *Display selection* widget is described [below](#display).  In the
...
variables. For example if we select the `Christoforou 2011`, the
...
(1) **_symbol type_**: By selecting one of the variable names in the
drop-down list of **_symbol type_** the symbol type of the features in
...
at, so we select "4" in the drop-down list **_number of plots to
display_**.  We will select `markers` in the drop-down list
**_feature(s) in_** and select `ER` (coding for endoplasmic reticulum)
...
`andy2011`, pd.markers` in the first drop-down list and `Golgi` as the
organelle to look at.  As the comparison we will choose `markers` and
`Golgi` in the drop-down lists of the second group which will display
the features in a transparent manner.
...
protein, e.g. OCAD1? The drop-down list for the variable name
...
just enter OCAD1 in the text input field **_Search for_** in between
the two drop-down lists and we will get the protein of interest (we
are also able to query for protein names which have the string `OC` in
