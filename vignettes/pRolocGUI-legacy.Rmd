--- 
title: "pRolocGUI - The legacy applications" 
author: "Thomas Naake and Laurent Gatto" 
output:
  BiocStyle::html_document 
vignette: >
  %\VignetteIndexEntry{pRolocGUI - The legacy applications} 
...
This vignette describes the functionality of the `r
Biocpkg("pRolocGUI")` legacy applications: `pRolocVis_legacy` and
`pRolocVis_compare`. A suite of newer applications are now available
in the `r Biocpkg("pRolocGUI")` package, please see
`vignette("pRolocGUI")` for details.

Key features of `pRolocVis_legacy` and `pRolocVis_compare` are the
identification of features in plots, a 'reverse search' based on
querying meta-data which allows for highlighting the features on plots
and an import/export functionality by using the `FeaturesOfInterest`/
`FoICollection` infrastructure distributed by the `MSnbase`
package. Additionally, `pRolocVis_compare` allows for comparison of
two comparable `MSnSet` instances, e.g. this might be of great help
for analyses of changes in protein localisation in different
`MSnSet`s.
...
The function `pRolocVis_legacy` needs an object of class `MSnSet` or a
list of `MSnSet` objects as an argument, while `pRolocVis_compare`
needs a list containing two instances of class `MSnSet`.  To prepare
the environment to run a `pRolocVis_legacy`/`pRolocVis_compare`
session, the `pRolocGUI` package and for demonstration purposes four
example `MSnSet`s are loaded to the environment. The example data sets
are available from the `pRolocdata` experiment package and are derived
from experiments `andy2011`, `tan2009r1` and `tan2009r2`, the first
and second replicate from and `dunkley2006`.
...
`pRolocVis_legacy` needs an object of class `MSnSet` as an argument. We can launch the
application with an `MSnSet` by assigning it to the argument `object`:
```{r, eval = FALSE, echo = TRUE}
pRolocVis_legacy(object = andy2011)
```

Alternatively, to upload multiple objects of class `MSnSet`, `pRolocVis_legacy` 
...
pRolocVis_legacy(object = namedVis)
pRolocVis_legacy(object = unnamedVis)
```

`pRolocVis_compare` requires a list of two `MSnSet`s which can be named or unnamed. 
```{r, eval = FALSE, echo = TRUE}
namedComp <- list(tan2009r1 = tan2009r1, tan2009r2 = tan2009r2)
unnamedComp <- list(tan2009r1, tan2009r2)
pRolocVis_compare(object = namedComp)
pRolocVis_compare(object = unnamedComp)
```

N.B.! It is also possible to use a partly named list in `pRolocVis_legacy` and 
`pRolocVis_compare`.

Launching `pRolocVis_legacy` or `pRolocVis_compare` will open a new tab in your default 
Internet browser.  
...
browser tab, where `pRolocVis_legacy`/`pRolocVis_compare` is running.

# Tabs of `pRolocVis_legacy` and `pRolocVis_compare`

To optimise ease of use the interfaces of `pRolocVis_legacy` and `pRolocVis_compare` are
...
In general, the tabs for `pRolocVis_legacy` and `pRolocVis_compare` will look alike. The tab 
**_data_** however differs for the two applications. While in `pRolocVis_legacy` this 
tab allows for the selection of the used `MSnSet` and the upload of `.Rda` or 
`.Rdata` files, in `pRolocVis_compare` there is the possibility to subset the used 
...
align="center"> Fig. 1: Vignette of `pRolocVis_legacy` and `pRolocVis_compare` </p>

## PCA

The tab **_PCA_** is characterised by its main panel which shows a
PCA plot for the selected `MSnSet` in the case of `pRolocVis_legacy`
and two PCA plots for `pRolocVis_legacy`. The sidebar panel is divided into 
*Display selection* and *Plot*.

<img src="figures/Screenshot_PCA1.png" width="100%" alt="" /> <p 
align="center"> Fig. 2: Appearance of **_PCA_** tab for `pRolocVis_legacy` (`andy2011`)</p>

For `pRolocVis_compare` the plot whose appearance is going to altered has to be selected
by selecting the appropriate [radio button](#RadioButtons) left of the name of 
the `MSnSet` instance in the sidebar panel. 

In addition, `pRolocVis_compare` offers the possibility to mirror the PCA plot of the 
...
`pRolocVis_compare`. Argument `object` is a named list with `MSnSet` instances 
`tan2009r1` and `tan2009r2`</p>

The manipulation of the plots works the same way for `pRolocVis_legacy` and for 
`pRolocVis_compare`:
...
variables. For example if we upload the `andy2011` data set in `pRolocVis_legacy`, 
...
For `pRolocVis_legacy` the tab **_protein profiles_** shows the protein profiles in 
the main panel (with an option of exporting the plot as it is shown in the main
panel by clicking on the button **_Download Plot_**) - for `pRolocVis_compare` it shows
...
panel (in `pRolocVis_compare` one has first to select the appropriate 
...
plasma membrane for the `andy2011` MSnSet. This is done in `pRolocVis_legacy`, but 
works in the same way in `pRolocVis_compare`. We have four organelles to look
at, so we select `4` (in `pRolocVis_compare` there is either the possibility to 
...
align="center"> Fig. 5: Appearance of **_protein profiles_** tab in `pRolocVis_legacy`
...
(see [3. *Display selection*](#display) for further details). In `pRolocVis_compare` 
...
(`andy2011`) in `pRolocVis_legacy`. Features shown originate from selection made in 
...
for further details). In `pRolocVis_compare` there is in addition a set of two 
...
In `pRolocVis_compare` select the appropriate [radio button](#RadioButtons) next to 
...
(`andy2011`) in `pRolocVis_legacy`</p>

## Search  

The appearance and operation are identical for `pRolocVis_legacy` and `pRolocGUI`.

`pRolocVis_legacy` and `pRolocVis_compare` allow to use past search results to display in the 
...
`pRolocVis_legacy`/`pRolocVis_compare` and its content is displayed in the tab **_search_**. 
...
exiting `pRolocVis_legacy`/`pRolocVis_compare`. 
...
When exiting `pRolocVis_legacy`/`pRolocVis_compare` the `FoICollection` will be assigned to 
...
usable by `pRolocVis_legacy`/`pRolocVis_compare`.
...
`pRolocVis_legacy`. Search result `pRolocVis_legacy_Test1` contains one feature of interest 
(PGAP1_Human)</p>
...
The tab **_data_** is the last tab for `pRolocVis_legacy` and `pRolocVis_compare`.

<img src="figures/Screenshot_Data1.png" width="100%" alt="" /> <p 
align="center"> Fig. 10: Appearance of **_data_** tab in `pRolocVis_legacy` </p>

For `pRolocVis_legacy` the drop-down menu lists all the names of assigned
...
`MSnSet` and load it to `pRolocVis_legacy`. Make sure if you want to use a
`MSnSet` from a `.Rda` or `.Rdata` file to select `upload` in the
drop-down menu.

<img src="figures/Screenshot_Data2.png" width="100%" alt="" /> <p 
align="center"> Fig. 11: File upload in `pRolocVis_legacy` </p>

`pRolocVis_legacy` will print a message if there are any conflicts with the
...
When analysing multiple data sets in one `pRolocVis_legacy` session the selected 
...
`MSnSet` (see [2.6. search](#tabspRolocVis_legacySearch) for further details).

<a id="tabspRolocGUIDataComp"></a>

For `pRolocVis_compare` the main panel shows a summary matrix of common and unique
...
`tan2009r1` and `tan2009r2` as input for `pRolocVis_compare`, when selecting `markers`
...
align="center">Fig. 12: Appearance of **_data_** tab in `pRolocVis_compare`. The row 
`ER` and column `unique1` which comprises `4` features are selected</p>

The tab **_Data_** in `pRolocVis_compare`also allows for subsetting the data sets by 
...
in `pRolocVis_legacy`/`pRolocVis_compare` and allows for identifying features. 
...
features in the `MSnSet` data. In `pRolocVis_compare`there is in addition the 
possibility to retrieve features from the summary matrix in the tab `data`.

There are four (`pRolocVis_legacy`) or five (`pRolocVis_compare`) check boxes in the
...
Selecting features works also in `pRolocVis_compare`: just click on features in one of 
...
(`andy2011`) for `pRolocVis_legacy`. The features selected originate from selection 
in the PCA plot </p>
...
profiles plot for `pRolocVis_legacy`.  The features selected originate from 
...
For `pRolocVis_compare` selecting features works in the same way: click on features in 
...
profiles plot for `pRolocVis_legacy`.  The features selected originate from selection 
...
In `pRolocVis_legacy`/`pRolocVis_compare` there is no functionality implemented to remove 
...
## Summary matrix (`pRolocVis_compare` only)

In `pRolocVis_compare` another way of selecting and displaying features is possible
...
For demonstration purposes we will use `pRolocVis_legacy` to select and display 
...
`MSnSet` when using `pRolocVis_compare`: Accordingly to the selected `MSnSet` the list 
...
align="center"> Fig. 16: Query for proteins in `pRolocVis_legacy` which contain the 
