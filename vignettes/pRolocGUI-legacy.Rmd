...
library(bibtex)
extbib <- read.bib("pRolocGUI-legacy.bib")
```

# Foreword

This vignette describes the functionality of the 
`r Biocpkg("pRolocGUI")` legacy applications: `pRolocVis_legacy` 
and `pRolocVis_compare`. A suite of newer applications 
are now available in the `r Biocpkg("pRolocGUI")` package, please
see `vignette("pRolocGUI")` for details.

Key features of `pRolocVis_legacy` and `pRolocVis_compare` 
...
distributed by the `MSnbase` package. Additionally, `pRolocVis_compare` allows for 
...
a list of `MSnSet` objects as an argument, while `pRolocVis_compare` needs a list 
containing two instances of class `MSnSet`. 
To prepare the environment to run a `pRolocVis_legacy`/`pRolocVis_compare` session, 
