...
extbib <- read.bib("Vignette_citations.bib")
...
defininitions of 
`MSnbase` `r citep(extbib["GattoLilley2012"])`
and on the functions defined in the package 
`pRoloc` 
`r citep(list(extbib["Breckels2013"], extbib["Gatto2014"]))`. 
It is especially meant for the analyses of
LOPIT `r citep(extbib["Dunkley2006"])`
or PCP `r citep(extbib["Foster2006"])` experiments.  To achieve reactivity
...
experiments `andy2011` from `r citet(extbib["Breckels2013"])`, 
`dunkley2006` from `r citet(extbib["Dunkley2006"])`, 
and `tan2009r1`, the first replicate from `r citet(extbib["Tan2009"])`, 
all provided by the package 
`pRolocdata` `r citep(extbib["GattoPRolocdata"])` or
