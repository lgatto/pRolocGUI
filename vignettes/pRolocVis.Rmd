...
in the package `pRoloc` [[2](#id2)]. It is especially meant for the
...
invited to contact Thomas Naake (tn299@cam.ac.uk) in case you have any 
questions, suggestions or have found any bugs or typos. To reach a broader 
audience for more general questions about proteomics analyses using R 
consider of writing to the Bioconductor list. 
...
sidebar, the *Display selection* widget (see 
...
Dunkley et al. (2006) [[3]](#id3), Tan et al. (2009) [[6]](#id6) provided 
by the package `pRolocdata`) or
...
The *Display selection* widget is [later](#display) on described. 
In the *Plot* compartiment there are possibilities to adjust the appearance of
...
list **_colour_**. For example if we select the `MSnSet` originating from
Breckels et al. (2013) the colour "markers", the features in the PCA plot
...
**_symbol type_** the symbol type of the features in
the plot will be changed.  
(2) **_legend_** and **_position of legend_**: By clicking on the check box 
to the left of **_legend_** a legend is added to the plot and 
by choosing one of the items in the drop-down list 
**_position of legend_** below its position will be changed.  
(3) **_point size_**: This drop-down list might appear when
`pRolocVis` has identified numeric values in the feature variable labels.
The default 1 allows for an unaltered display of the plot, while selecting 
other items in the list renders the features in the PCA plot according to 
their numerical value in the variable label (this might be of interest 
if you have scores which give information about the quality of a 
phenoDisco analysis).

By changing the drop-down lists of the items 
**_number of 1st principal component_** and 
**_number of 2nd principal component_** the x-values and y-values, 
...
Breckels et al. (2013).  We have four organelles to look at, so we select "4"
in the drop-down list **_number of plots to display_**. 
We will select `markers` in the drop-down list **_feature(s) in_** and 
select `ER` (coding for
endoplasmic reticulum) in the drow-down list underneath (**_assigned to_**) 
in the first group. The other two drop-down list will be left as they are.
To display the next plot we have to
change the slider **_Selected plot_** to position 2
...
displayed, accordingly to our question we will change the second
...
lists addordingly to the organelles we want to display. Please be 
aware that it is possible to "go" back to a plot to change its 
parameter.

We can change the two drop-down lists in the second group if we want to 
display the protein profiles compared to protein profiles assigned to another 
variable name (or by default the same). The features selected in the drop-down 
...
displayed.
To display results of the `phenoDisco` function we can select 
for the `MSnSet` from Breckels et al. (2013) `pd.markers` in the 
first drop-down list and `Golgi` as the organelle to look at.  As the 
comparison we will choose `markers` and `Golgi` in the drop-down 
lists of the second group which will display the features in a 
transparent manner.
...
is derived from the experiment of Breckels et al. (2013) but will have no
...
experiments of Breckels et al. (2013) for all proteins which are assigned by
experimental evidence to the organelle `plasma membrane`. We ensure
ourselves that `Breckels et al. (2013)` is selected in the tab **_Data_**
...
in the upper drop-down list (for we are looking for organelles). In
...
from the experiments of Breckels et al. (2013)). But how should we proceed
...
URL: http://www.ncbi.nlm.nih.gov/pubmed/22113085
