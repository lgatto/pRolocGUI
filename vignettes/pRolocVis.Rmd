...
with the underlying spatial proteomics data: The distributed function 
`pRolocVis` offers interactive Principal Component Analysis (PCA) plots and 
protein profile plots, as well as exploration of quantitative and 
qualitative meta-data. 
Key features of `pRolocVis` are the identification of features in 
plots, a 'reverse search' based on querying meta-data which allows for 
highlighting the features on plots and an import/export functionality by 
using the `FeaturesOfInterest`/`FoICollection` infrastructure distributed 
by the `MSnbase` package. 
...
Currently, `pRolocGUI` distributes the `pRolocVis` application.  

The function `pRolocVis` needs an object of class `MSnSet` or 
a list of `MSnSet` objects as an argument. 
To prepare the environment to run a `pRolocVis` session, the `pRolocGUI` 
package and for demonstration purposes three example `MSnSet`s are loaded to the 
environment. The example data sets are available from the 
`pRolocdata` [`r citep(extbib["GattoPRolocdata"])`](#references) experiment
package and are derived from experiments `andy2011` from 
[`r citet(extbib["Breckels2013"])`](#references), `dunkley2006` from 
[`r citet(extbib["Dunkley2006"])`](#references), and `tan2009r1`, 
the first replicate from [`r citet(extbib["Tan2009"])`](#references). 
...
of the data without stopping the application from running. 
...
Launching the function `pRolocVis` will open a new tab in your default 
Internet browser.  
...
To optimise ease of use the interface of `pRolocVis` is subdivided in 
seven tabs:  
...
When analyzing multiple data sets in one `pRolocVis`session errors can occure
when changing from one `MSnSet` to the other. It is therefore highly recommended
to delete all selected features before changing to another `MSnSet` by 
clicking on **_Clear features_** in the *Display selection* widget. 
...
features_** (the button will only show up if features were already selected).
