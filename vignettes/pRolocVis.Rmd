...
MSnSets (derived from experiments of Breckels 2013, Dunkley et
al. 2006, Tan et al. 2009) provided by the package **pRolocdata** or
...
Christoforou 2011 the colour "markers", the features in the PCA plot
...
and for the shiny reactive expressions didn't receive yet a new input
...
name. To see display results of phenoDisco we can select for the
`MSnSet` from Breckels 2013 `pd.markers` in the first drop-down list
...
details if this is your intention).  This ability requires an object
...
ourselves that `Christoforou 2011` is selected in the tab **_Data_**
and change to a tab where the *Display selection* widget is loaded. We
activate the check box to the left of **_level_** and select "marker"
...
membrane". Next, we click on **_Submit selected level_**, which will
...
selected level_** to save internally the selected features.
...
from the experiments of Breckels (2011)). But how should we proceed
...
clicking on **_Submit selected level_** we save internally the
selected feature(s).
...
<a id="id2"></a>
[2] Gatto L, Breckels LM, Wieczorek S, Burger T, Lilley
...
map by protein correlation profiling. Cell, 125(1):187-199, Apr 2006.
