...
%\VignetteIndexEntry{pRolocVis_legacy and pRolocVis_legacy_compare application}
...
<h1>pRolocGUI: Interactive visualisation of spatial proteomics data using the legacy models</h1>
...
`pRolocVis_legacy` and `pRolocVis_legacy_compare` offer interactive 
...
Key features of `pRolocVis_legacy` and `pRolocVis_legacy_compare` are the identification of 
...
distributed by the `MSnbase` package. Additionally, `pRolocVis_legacy_compare` allows for 
...
Currently, `pRolocGUI` distributes the `pRolocVis_legacy` and `pRolocVis_legacy_compare`application.  

The function `pRolocVis_legacy` needs an object of class `MSnSet` or 
a list of `MSnSet` objects as an argument, while `pRolocVis_legacy_compare` needs a list 
containing two instances of class `MSnSet`. 
To prepare the environment to run a `pRolocVis_legacy`/`pRolocVis_legacy_compare` session, 
...
`pRolocVis_legacy_compare` requires a list of two `MSnSet`s which can be named or unnamed. 
```{r, eval = FALSE, echo = TRUE}
namedComp <- list(tan2009r1 = tan2009r1, tan2009r2 = tan2009r2)
unnamedComp <- list(tan2009r1, tan2009r2)
pRolocVis_legacy_compare(object = namedComp)
pRolocVis_legacy_compare(object = unnamedComp)
```

N.B.! It is also possible to use a partly named list in `pRolocVis_legacy` and 
`pRolocVis_legacy_compare`.

Launching `pRolocVis_legacy` or `pRolocVis_legacy_compare` will open a new tab in your default 
Internet browser.  
...
browser tab, where `pRolocVis_legacy`/`pRolocVis_legacy_compare` is running.

-------------------
<a id="tabspRolocGUI"></a> 

<h2>2. Tabs of <code>pRolocVis_legacy</code> and <code>pRolocVis_legacy_compare</code></h2>

To optimise ease of use the interfaces of `pRolocVis_legacy` and `pRolocVis_legacy_compare` are
...
In general, the tabs for `pRolocVis_legacy` and `pRolocVis_legacy_compare` will look alike. The tab 
...
`.Rdata` files, in `pRolocVis_legacy_compare` there is the possibility to subset the used 
...
align="center"> Fig. 1: Vignette of `pRolocVis_legacy` and `pRolocVis_legacy_compare` </p>
...
For `pRolocVis_legacy_compare` the plot whose appearance is going to altered has to be selected
by selecting the appropriate [radio button](#RadioButtons) left of the name of 
the `MSnSet` instance in the sidebar panel. 

In addition, `pRolocVis_legacy_compare` offers the possibility to mirror the PCA plot of the 
...
`pRolocVis_legacy_compare`. Argument `object` is a named list with `MSnSet` instances 
...
panel by clicking on the button **_Download Plot_**) - for `pRolocVis_legacy_compare` it shows
...
panel (in `pRolocVis_legacy_compare` one has first to select the appropriate 
...
works in the same way in `pRolocVis_legacy_compare`. We have four organelles to look
at, so we select `4` (in `pRolocVis_legacy_compare` there is either the possibility to 
...
(see [3. *Display selection*](#display) for further details). In `pRolocVis_legacy_compare` 
...
for further details). In `pRolocVis_legacy_compare` there is in addition a set of two 
...
In `pRolocVis_legacy_compare` select the appropriate [radio button](#RadioButtons) next to 
...
`pRolocVis_legacy` and `pRolocVis_legacy_compare` allow to use past search results to display in the 
...
`pRolocVis_legacy`/`pRolocVis_legacy_compare` and its content is displayed in the tab **_search_**. 
...
exiting `pRolocVis_legacy`/`pRolocVis_legacy_compare`. 
...
When exiting `pRolocVis_legacy`/`pRolocVis_legacy_compare` the `FoICollection` will be assigned to 
...
usable by `pRolocVis_legacy`/`pRolocVis_legacy_compare`.
...
The tab **_data_** is the last tab for `pRolocVis_legacy` and `pRolocVis_legacy_compare`.
...
For `pRolocVis_legacy_compare` the main panel shows a summary matrix of common and unique
...
`tan2009r1` and `tan2009r2` as input for `pRolocVis_legacy_compare`, when selecting `markers`
...
align="center">Fig. 12: Appearance of **_data_** tab in `pRolocVis_legacy_compare`. The row 
`ER` and column `unique1` which comprises `4` features are selected</p>

The tab **_Data_** in `pRolocVis_legacy_compare`also allows for subsetting the data sets by 
...
in `pRolocVis_legacy`/`pRolocVis_legacy_compare` and allows for identifying features. 
...
features in the `MSnSet` data. In `pRolocVis_legacy_compare`there is in addition the 
possibility to retrieve features from the summary matrix in the tab `data`.

There are four (`pRolocVis_legacy`) or five (`pRolocVis_legacy_compare`) check boxes in the
...
Selecting features works also in `pRolocVis_legacy_compare`: just click on features in one of 
...
For `pRolocVis_legacy_compare` selecting features works in the same way: click on features in 
...
In `pRolocVis_legacy`/`pRolocVis_legacy_compare` there is no functionality implemented to remove 
...
<a id="displayData"></a>
### 3.4. summary matrix (`pRolocVis_legacy_compare` only)

In `pRolocVis_legacy_compare` another way of selecting and displaying features is possible
...
`MSnSet` when using `pRolocVis_legacy_compare`: Accordingly to the selected `MSnSet` the list 
